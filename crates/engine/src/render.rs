    ...
    let result=async {
        if supervisor.is_some() {
            ...
            tokio::time::timeout(Duration::from_secs(30),async {
                loop {
                    ...
                    match lock.try_lock_exclusive() { Ok(())=>break,Err(e) if e.raw_os_error()==fs2::lock_contended_error().raw_os_error()=>tokio::time::sleep(Duration::from_millis(100)).await,Err(e)=>return Err(e.into()) }
                } Ok::<_,anyhow::Error>(())
            }).await.context("이전 미디어 작업 종료 대기 시간 초과")??;
        }
        if std::fs::symlink_metadata(&temp).is_ok_and(|m|!m.is_file()) {bail!("임시 미디어 파일 경로 오류");}
        if temp.exists(){std::fs::remove_file(&temp)?;}
        ...
        if preview {
            if !options.audio_only {
                ...
                enc.args(["-hide_banner","-encoders"]).kill_on_drop(true); process::hide_console(&mut enc);
        ...
        if let Some(supervisor)=supervisor {
            let spec=process::WorkerSpec{executable:tools.require("ffmpeg")?,args:cmd.as_std().get_args().map(|s|s.to_string_lossy().into_owned()).collect(),cwd:worker_dir.clone()};
        ...
        let errors=tokio::spawn(async move {
            let mut reader=BufReader::new(stderr); let mut output=Vec::new(); let mut buf=[0u8;1024];
        ...
        let mut bytes=0; let mut seconds=0.0;
        let status=tokio::select! {
            _=cancel.cancelled()=> {
                lifetime_pipe.take();
                if supervisor.is_none() {let _=child.kill().await;}
                if tokio::time::timeout(Duration::from_secs(30),child.wait()).await.is_err(){let _=child.kill().await;}
                errors.abort();bail!("작업이 중단되었습니다.");
            }
            result=async {
                while let Some(line)=lines.next_line().await? {
                    ...
                    if line.starts_with("progress=") {
                        if available_space(parent)? < min_free_bytes {
                            lifetime_pipe.take();
                            if supervisor.is_none(){let _=child.kill().await;}
                            let _=tokio::time::timeout(Duration::from_secs(30),child.wait()).await;
                            bail!("저장 공간 부족으로 미디어 작업을 중단했습니다.");
                        }
        ...
        if !status.success() { bail!("미디어 처리 실패: {error}"); }
        if cancel.is_cancelled() { bail!("작업이 중단되었습니다."); }
        let mut media=probe(tools,&temp).await?;
        if !options.accepts(&media) || media.duration<=0.0 { bail!("결과 파일 검사 실패"); }
        let path=temp.clone();
        progress(media.bytes,-1.0);
        let check=cancel.clone();
        let digest=tokio::task::spawn_blocking(move||file_digest_with_progress(&path,|_|{if check.is_cancelled(){bail!("작업이 중단되었습니다.");}Ok(())})).await??;
