...
pub async fn run(state: Arc<Service>) {
    ...
    loop {
        ...
        let mut due: Vec<_> = jobs.into_iter().filter(|j| j.attempt > 0).collect();
        due.sort_by_key(|job| {
            if job.backup.state.is_empty() || job.backup.verified_files == 0 {
                0u8
            } else if job.backup.state == "failed" {
                1
            } else if !job.state.terminal() {
                2
            } else if job.backup.state != "verified" {
                3
            } else {
                4
            }
        });
        let Some(job) = due.into_iter().find(|job| {
            let wait = if job.state.terminal() && job.backup.state == "verified" {
                300
            } else if job.backup.state == "failed" || job.backup.verified_files == 0 {
                2
            } else {
                10
            };
        ...
        let result = async {
            ...
            let output = tokio::time::timeout(Duration::from_secs(180), child.wait_with_output())
                .await
                .map_err(|_| anyhow::anyhow!("백업 작업 시간 제한 초과 · 다음 주기에 재시도"))??;
            if !output.status.success() {
                anyhow::bail!("{}", redact(&String::from_utf8_lossy(&output.stderr)));
            }
            Ok::<usize, anyhow::Error>(serde_json::from_slice(&output.stdout)?)
        };
        let result = tokio::select! { _ = state.shutdown.cancelled() => return, r = result => r };
